...
<div class="fieldcontain ${hasErrors(bean: shareFileInstance, field: 'users', 'error')} ">
    <label for="users">
        <g:message code="shareFile.users.label" default="Users"/>

    </label>
    <g:select name="users" from="${com.sharefile.auth.User.findAllByUsernameNotEqual(session.user?.username)}" multiple="multiple" optionKey="id" size="5"
