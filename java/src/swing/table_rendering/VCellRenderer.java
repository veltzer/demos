...
public class VCellRenderer extends JLabel implements Border, TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table,
        ...
        if(value == null) {
            ...
            if(value instanceof Date) {
                ...
                if(value instanceof Currency) {
                    ...
                    NumberFormat format = NumberFormat.getCurrencyInstance(currency.getType());
        ...
        TableAlignmentModel alignmentModel = ((CTable)table).getAlignmentModel();
    ...
    public void paintBorder(Component component, Graphics graphics, int param, int param3, int param4, int param5) {
        graphics.setColor(gridColor);
        graphics.drawLine(0, getHeight() - 1, getWidth(), getHeight() - 1);
        graphics.drawLine(getWidth() - 1, 0, getWidth() - 1, getHeight() - 1);
    }
     ...
     */
    private BufferedImage img = new BufferedImage(300, 300, BufferedImage.TYPE_INT_RGB);
    
    private Map<Cell, JLabel> cells = new HashMap<Cell, JLabel>();
    
    public JComponent span(int row, int column, JComponent cmp, SpanModel model, JTable table) {
        if(model.isSpanRoot(row, column)) {
            ...
            if(width > img.getWidth() || height > img.getHeight()) {
                img = new BufferedImage(Math.max(img.getWidth(), width), Math.max(img.getHeight(), height), 
            ...
            for(int cCell = column ; cCell <= spanColumns ; cCell++) {
                ...
                for(int rCell = row ; rCell <= spanRows ; rCell++) {
                    ...
                    BufferedImage currentImage = img.getSubimage(x, y, width, height);
