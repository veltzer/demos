...
public class AddressBookBrowser extends ViewPart {
	...
	public void createPartControl(Composite parent) {
        ...
        nameColumn.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                 viewer.setSorter(
                     new AddressBookSorter(AddressBookTableLabelProvider.NAME_COLUMN, 
        ...
        numberColumn.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                 viewer.setSorter(
                     new AddressBookSorter(AddressBookTableLabelProvider.NUMBER_COLUMN, 
        ...
        ((Text)numberEditor.getControl()).addVerifyListener(new VerifyListener() {
	...
	private void makeActions() {
		deleteAction = new Action("Delete") {
			public void run() {
                ...
                for (int i = 0; i < selected.length; i++) {
                    AddressBookManager.getInstance().remove(selected[i].getData());
                }
                viewer.refresh();
			}
		};
        deleteAction.setEnabled(!viewer.getSelection().isEmpty());
		deleteAction.setToolTipText(Messages.getString("AddressBookBrowser.delete.tooltip")); //$NON-NLS-1$
        ...
        editAction.setToolTipText(Messages.getString("AddressBookBrowser.edit.tooltip")); //$NON-NLS-1$
        ...
        newAction = new Action(Messages.getString("AddressBookBrowser.new")) { //$NON-NLS-1$
            public void run() {
                ...
                Object newRecord;
                if (newCompany) {
                    newRecord = new Company(Messages.getString("AddressBookBrowser.company.default"), ""); //$NON-NLS-1$ //$NON-NLS-2$
                    AddressBookManager.getInstance().addCompany((Company)newRecord);
                }
                else {
                    newRecord = new Person(Messages.getString("AddressBookBrowser.friend.default.1"), Messages.getString("AddressBookBrowser.friend.default.2"), ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    AddressBookManager.getInstance().addFriend((Person)newRecord);
                }
        ...
        newAction.setToolTipText(Messages.getString("AddressBookBrowser.new.tooltip")); //$NON-NLS-1$
        ...
        class ToggleFilterAction extends Action {
            ...
            public void run() {
                try {
                    ...
                    filter = new AddressBookViewerFilter(showCompanies.isChecked(), showFriends.isChecked());
                    viewer.addFilter(filter);
                }
                finally {
        ...
        showCompanies = new ToggleFilterAction(Messages.getString("AddressBookBrowser.show.companies")); //$NON-NLS-1$
        showCompanies.setImageDescriptor(ViewPlugin.getCompanyIconDescriptor());
        showCompanies.setChecked(true);
        showCompanies.setToolTipText(Messages.getString("AddressBookBrowser.show.companies.tooltip")); //$NON-NLS-1$
        
        showFriends = new ToggleFilterAction(Messages.getString("AddressBookBrowser.show.friends")); //$NON-NLS-1$
        showFriends.setImageDescriptor(ViewPlugin.getFriendIconDescriptor());
        showFriends.setChecked(true);
        showFriends.setToolTipText(Messages.getString("AddressBookBrowser.show.friends.tooltip")); //$NON-NLS-1$
	}
