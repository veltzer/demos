...
public class DvdCatalog extends JFrame implements Runnable {
    ...
    public DvdCatalog() throws IOException {
        ...
        for(int iter = 0 ; iter < LF_COMPONENTS.length ; iter++) {
            UIManager.put(LF_COMPONENTS[iter] + ".font", font);
        }
        ...
        btn.addActionListener(
            new ActionListener() {
                public void actionPerformed(ActionEvent ev) {
                    tableModel.add(title.getText(), by.getText(), featuring.getText(),
                            year.getText(), keywords.getText(), dvd.isSelected(), cd.isSelected());
                    title.setText("");
                    by.setText("");
                    featuring.setText("");
                    year.setText("");
    ...
    public static class DvdModel extends AbstractTableModel implements TableModel, Serializable, ExceptionListener {
        /**
		 * 
		 */
		private static final long serialVersionUID = 1L;
		private final transient String[] COLUMN_NAMES = {"Title", "By", "Featuring", "Year", "Keywords", "Media"};
        private final transient Class<?>[] COLUMN_CLASSES = {String.class, String.class, String.class, String.class, String.class, Integer.class};
        ...
        public void add(String title, String by, String featuring, String year, String keywords, boolean isDvd, boolean isCd) {
            if (isDvd) {
                tableData.add(new Object[]{title, by, featuring, year, keywords, new Integer(0)});
            } else {
                if (isCd) {
                    tableData.add(new Object[]{title, by, featuring, year, keywords, new Integer(1)});
                } else {
                    tableData.add(new Object[]{title, by, featuring, year, keywords, new Integer(2)});
                }
            }
