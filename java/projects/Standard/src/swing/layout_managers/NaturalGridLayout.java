...
public class NaturalGridLayout implements LayoutManager {
    private int rows;
    private int columns;
    private final static GetSize PREFERRED_SIZE =
        new GetSize() {
    ...
    private final static GetSize MINIMUM_SIZE =
        new GetSize() {
            public Dimension getSize(Component cmp) {
    ...
    public void layoutContainer(Container parent) {
        boolean rToL = ComponentOrientation.RIGHT_TO_LEFT.equals(parent.getComponentOrientation());
        Insets insets = parent.getInsets();
        Dimension size = parent.getSize();

        int componentWidth = (size.width - (insets.left + insets.right)) / columns;
        int componentHeight = (size.height - (insets.top + insets.bottom)) / rows;
        ...
        for (int rowIter = 0; rowIter < rows; rowIter++) {
            for (int columnIter = 0; columnIter < columns; columnIter++) {
                ...
                placeComponent(rToL, comp, rowIter, columnIter, componentWidth, componentHeight);
                currentComponent++;
    ...
    private void placeComponent(boolean rToL, Component comp, int row, int column, int componentWidth, int componentHeight) {
