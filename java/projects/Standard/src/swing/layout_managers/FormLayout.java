...
public final class FormLayout implements LayoutManager2, Serializable {
	...
	/**
	 * Constructs an instance of <code>FormLayout</code> using the given column
	...
	private void initializeColAndRowComponentLists() {
		...
		for (int i = 0; i < getColumnCount(); i++) {
			colComponents[i] = new LinkedList<Component>();
		}

		rowComponents = new LinkedList[getRowCount()];
		for (int i = 0; i < getRowCount(); i++) {
			rowComponents[i] = new LinkedList<Component>();
		}
	...
	private int[] computeGridOrigins(Container container, int totalSize,
			int offset, List<FormSpec> formSpecs,
			List<Component>[] componentLists, int[][] groupIndices,
	 ...
	 */
	@SuppressWarnings("unchecked")
	private int[] maximumSizes(Container container, List<FormSpec> formSpecs,
			@SuppressWarnings("rawtypes") List[] componentLists,
	...
	/**
	 * An interface that describes how to measure a <code>Component</code>. Used
	 ...
	 * totalWidth); int totalHeight = 0; System.out.print("Row heights: ");
	 * for (int i=0; i < getRowCount(); i++) { int height = rowHeights[i];
	 * totalHeight += height; System.out.print(height + ", "); }
	 * System.out.println(" Total=" + totalHeight); System.out.println(); }
	 */
