...
public class FTable extends JSplitPane {
    ...
    public FTable(TableModel model, int[] frozenColumns) {
        ...
        JTable frozenTable = new JTable(new HiddenColumnsProxy(model, frozenColumns));
        JTable notFrozenTable = new JTable(new HiddenColumnsProxy(model, notFrozen));
        ...
        frozenPane = new JScrollPane(frozenTable, JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
        notFrozenPane = new JScrollPane(notFrozenTable, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
        setOrientation(HORIZONTAL_SPLIT);
        setLeftComponent(frozenPane);
        setRightComponent(notFrozenPane);
        notFrozenPane.getVerticalScrollBar().addAdjustmentListener(new AdjustmentListener () {
            public void adjustmentValueChanged(java.awt.event.AdjustmentEvent ev) {
                frozenPane.getVerticalScrollBar().setValue(ev.getValue());
            }
        });
        frozenPane.getVerticalScrollBar().addAdjustmentListener(new AdjustmentListener () {
            public void adjustmentValueChanged(java.awt.event.AdjustmentEvent ev) {
                notFrozenPane.getVerticalScrollBar().setValue(ev.getValue());
            }
        });
