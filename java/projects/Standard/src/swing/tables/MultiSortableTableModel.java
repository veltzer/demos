...
public class MultiSortableTableModel extends ProxyTableModel {
    ...
    public void sort(int[] columns, boolean[] ascending) {
    	ArrayList<Comparator<Object>> comparators=new ArrayList<Comparator<Object>>(columns.length);
        for(int compIter = 0 ; compIter < columns.length ; compIter++) {
            if(Number.class.isAssignableFrom(getColumnClass(columns[compIter]))) {
        ...
        Collections.sort(rows, new TableComparator(comparators, columns, ascending));
    ...
    class TableComparator implements Comparator<Object> {
        ...
        public TableComparator(ArrayList<Comparator<Object>> comparators, int[] columns, boolean[] ascending) {
        ...
        public int compare(Object o1, Object o2) {
            ...
            for(int iter = 0 ; iter < comparators.size() ; iter++) {
                value = comparators.get(iter).compare(getRowData(o1, iter), getRowData(o2, iter));
                if(value != 0) {
                    if(!ascending[iter]) {
                        return(value *= -1);
                    }
