...
<HTML>
	...
	<BODY> 
		...
		<TABLE BORDER="1">
			...
			<TR><TD>10%3 (mod)</TD><TD>${10%3}</TD></TR>
			<TR><TD>2>1&&"ab"&gt;"aaa"</TD><TD>${10>2 && "z" gt "a"}</TD></TR>
			<TR><TD>empty null && empty "" && !empty "a"</TD><TD>${empty null && empty "" && !empty "a"}</TD></TR>
			<TR><TD>true?1:2</TD><TD>${true?1:2}</TD></TR>
		</TABLE>
		<P> 
		...
		<TABLE BORDER="1">			
			...
			<TR>
				...
				<TD>
<c:forEach var="c" items="${paramValues['colors']}">
...
<c:forEach var="c" items="${cookie}">
	  				<li>name ${c.key}
	  				<br>domain ${c.value.domain}
	  				<br>value ${c.value.value}
</c:forEach>
				</TD>
			</TR>
		</TABLE>
		...
		<TABLE BORDER="1">
			<TR><TH>Expression</TH><TH>Value</TH></TR>
			<TR><TD>sin(11/7)</TD><TD>${f:sin(11/7)}</TD></TR>
		</TABLE>
	</BODY>
</HTML>
